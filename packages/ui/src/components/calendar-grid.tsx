  ...
  function CalendarGrid(
    ...
    const rovingTarget =
      inViewMonth(focusedDate) && !isDayDisabled(focusedDate)
        ? focusedDate
        : (gridDays.find((d) => inViewMonth(d) && !isDayDisabled(d)) ??
          gridDays.find((d) => inViewMonth(d)) ??
          gridDays[0]);
      ...
      switch (e.key) {
        ...
        case "PageUp": {
          ...
          const target = e.shiftKey
            ? addYearsClamped(rovingTarget, -1)
            : addMonthsClamped(rovingTarget, -1);
          moveFocusTo(isDayDisabled(target) ? step(target, -1) : target, true);
          break;
        }
        case "PageDown": {
          ...
          {Array.from({ length: 6 }, (_, week) => (
            <div role="row" key={week} className="grid grid-cols-7">
              {gridDays.slice(week * 7, week * 7 + 7).map((day) => {
                if (!inViewMonth(day)) {
                  return (
                    <div key={dateKey(day)} role="gridcell" aria-hidden="true" className="h-9" />
                  );
                }
