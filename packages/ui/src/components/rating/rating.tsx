...
export const Rating = React.forwardRef<HTMLDivElement, RatingProps>(
  (
    ...
    const getEventValue = (e: React.MouseEvent<HTMLButtonElement>, starIndex: number): number => {
      const target = e.currentTarget;
      const width = target.offsetWidth;
      const nativeEv = e.nativeEvent as any;
      const offsetX = nativeEv?.offsetX ?? nativeEv?.nativeEvent?.offsetX ?? (e as any).offsetX;
    ...
    return (
      ...
      >
                      ...
                      <defs>
                        <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="0%">
                          ...
                          {isCssColor(emptyColor) ? (
                            <stop offset={`${fillFraction * 100}%`} stopColor={emptyColor} />
                          ) : (
                            <stop
                              offset={`${fillFraction * 100}%`}
                              stopColor="currentColor"
        ...
        {showCount && count !== undefined && (
          <span className={cn("text-muted-foreground ml-1", sizing.text)}>({count})</span>
        )}
      </div>
    );
  }
