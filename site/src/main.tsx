...
function SidebarContent({
  ...
  const filteredNav = React.useMemo(() => {
    ...
    return NAV.filter(([id, label]) => id.toLowerCase().includes(clean) || label.toLowerCase().includes(clean));
  }, [searchQuery]);
  ...
  return (
    <div className="gallery-layout">
      <aside className="sticky top-0 self-start h-screen overflow-y-auto bg-surface border-r border-border p-6 max-[820px]:hidden flex flex-col gap-4">
        <SidebarContent searchQuery={searchQuery} setSearchQuery={setSearchQuery} activeSection={activeSection} onSelectSection={(id) => setActiveSection(id)} />
      </aside>

      <div className="min-w-0">
        <header
          className="sticky top-0 z-30 flex items-center justify-between gap-4 px-8 max-[820px]:px-5 py-4 border-b border-border"
          style={{ background: "color-mix(in oklch, var(--kj-background) 82%, transparent)", backdropFilter: "blur(12px)" }}
        >
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 flex" role="presentation" onClick={() => setSidebarOpen(false)}>
          <aside
          ...
          >
            <SidebarContent
              ...
              onSelectSection={(id) => { setActiveSection(id); setSidebarOpen(false); }}
            />
          </aside>
